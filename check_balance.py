...
def main():
    """
    Fonction principale pour charger les clés API, récupérer et afficher
    ...
    if not api_key or not api_secret:
        print("Erreur : Les clés API Binance (BINANCE_API_KEY, "
              "BINANCE_API_SECRET) ne sont pas configurées dans le "
              "fichier .env.")
        return
    ...
    # Vous pouvez spécifier une liste de stablecoins si vous le souhaitez,
    # par exemple : stablecoins_to_check = ['USDC', 'USDT']
    # Sinon, la fonction get_binance_balance utilisera sa liste par défaut.
    stablecoin_balances = get_binance_balance(api_key,
                                              api_secret,
                                              testnet=True)
    ...
    if stablecoin_balances:
        ...
        for asset, balance in stablecoin_balances.items():
            ...
            # Supposant une parité 1:1 avec une monnaie de référence (ex: USD)
            total_stablecoin_value += balance

        print(f"\nValeur totale approximative des stablecoins : "
              f"{total_stablecoin_value:.2f}")
    else:
        print("Aucun solde de stablecoin n'a été récupéré ou les soldes "
              "sont nuls pour les actifs par défaut.")
        print("Actifs par défaut vérifiés : ['USDC', 'USDT', 'BUSD', 'DAI', "
              "'TUSD', 'PAX']")
